...
- **预报对照**：按水库与时段把预报入库流量与实测入库流量并排列出，按日、按旬、按月三种口径给出绝对偏差、相对偏差与按量加权偏差；给出偏大偏小天数与占比、平均绝对相对偏差、命中率与是否达到合格线的结论，并列出预报偏差最大的若干天；判定范围可在页面上试算（改动前后对照）或保存为设置。

演示用的预报记录可用 `node scripts/seed-forecast.js` 生成（幂等，可重复执行）。
...
7. **预报与实测分离**：入库流量的 `type` 为「预报」时是预报记录，其余算实测。概览的当天入库、水位表的当天入库、水量平衡的入库水量都**只算实测**；「预报对照」页才把两类配对。同库同日的预报只留一条，重复登记直接覆盖。
8. **预报对照配对**：按 水库 + 日期 配对，同库同日多条各自求和；只有一边的天数（缺预报 / 缺实测）单独计数，不参与偏差统计。绝对偏差 = 预报 − 实测（正为偏大）；相对偏差 = 绝对偏差 ÷ 实测 × 100%；按量加权相对偏差 = Σ(实测水量 × |当日相对偏差|) ÷ Σ实测水量（水量按 86400 秒折万 m³）。旬按 1–10 / 11–20 / 21 起分上中下旬，旬与月只统计两边都有的天。
9. **命中与合格**：|预报 − 实测| ≤ max(`forecastToleranceFlow`，实测 × `forecastTolerancePct`%) 算命中；旬/月按偏差水量判定（允许绝对偏差水量 = `forecastToleranceFlow` × 86400 × 对照天数 ÷ 10000）。命中率 ≥ `forecastPassPct`% 判「达到」，否则「未达到」。同一批数据重复评估结果一致（接口返回内容指纹 SHA-1 供核对）；判定范围改动走 `/api/forecast/whatif`，同一批数据按改动前后各评一次并列出判定翻转的天。
...
| GET | /api/forecast/eval?reservoirId=&from=&to=&top= | 预报对照评估（按日/按旬/按月、统计、偏差最大天、内容指纹） |
| GET | /api/forecast/whatif?...&tolerancePct=&toleranceFlow=&passPct= | 同一批数据按改动前后的判定范围各评一次，给出对照与判定翻转清单 |
